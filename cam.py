...
#     model=SwinTransformer(img_size=448, num_classes=200)
    ...
    imgclass.sort()
    imglist = os.listdir(os.path.join(args.image_path, imgclass[args.image_class-1]))
    imglist.sort()
    print("lenlist={}".format(len(imglist)))
    for i in range(len(imglist)):
        imgname = imglist[i]
        rgb_img = cv2.imread(os.path.join(args.image_path, imgclass[args.image_class-1], imgname), 1)
        if rgb_img is None:
    ...
    #         print(y.shape)
            ...
            cv2.imwrite(os.path.join('./cam', f'{args.image_class:03}_{args.stage}_{args.block}_' + imgname[:-4] + f'_{args.method}_cam.jpg'), cam_image)